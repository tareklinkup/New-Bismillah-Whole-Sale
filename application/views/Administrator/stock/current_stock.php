        ...
        <div class="col-md-12">
            <div class="table-responsive" id="stockContent">
                <table class="table table-bordered" v-if="searchType == 'current'" style="display:none"
                    ...
                    <tbody>
                        <template v-for="category in uniqueCategories">
                            <tr>
                                <td colspan="6"
                                    style="text-align:center;background:#ddd;font-weight:bold;font-size:16px">
                                    {{ category }}
                                </td>
                            </tr>
                            <template v-for="product in stock">
                                <tr v-if="category == product.ProductCategory_Name">
    ...
    methods: {
        getStock() {
            ...
            axios.post(url, parameters).then(res => {
                ...
                if (this.searchType == 'current') {
                    // let resl = res.data.stock.map(a => a.ProductCategory_Name);
                    const unique = [...new Set(res.data.stock.filter((pro) => pro.current_quantity != 0)
                        .map(item => item.ProductCategory_Name))];
                    this.uniqueCategories = unique;
                    console.log(unique);
                    this.stock = res.data.stock.filter((pro) => pro.current_quantity != 0).sort(
                        (a, b) => {
                    ...
                    this.stock = res.data.stock.sort(
                        (a, b) => {
                            let fa = a.Product_Name.toLowerCase(),
                                fb = b.Product_Name.toLowerCase();
                            if (fa < fb) {
